...
def train(train_loader, model, Siamese_model, head, loss_fn, optimizer, params):
    ...
    for i, x in enumerate(tqdm(train_loader)):
        optimizer.zero_grad() 
        # 224 * 224 patches: crop 後再 resize 回 224
        ...
        pred_query_set = head(support_set_anchor, query_set) # (7*5*15,5) # 每一張圖片都有 6 張 patches 和 1 個 patches 的 anchor
        ...
        ce_loss = loss_fn(pred_query_set_anchor, query_set_y) # query set anchor 跟 support set prototype 做 matching 算 cls loss
        ...
        self_image_loss = torch.mean(torch.sum(torch.log((pred_query_set_aug + eps)**(-pred_query_set_anchor)), dim=1)) # prediction of global features and local features(6) should be similar
        
        ################# add local patches loss ###############
        rand_id = np.random.permutation(6) # random pick one image in the query set
        pred_query_set_local = pred_query_set_aug.view(6, params.n_way, params.n_query, params.n_way)
        pred_query_set_local_1 = pred_query_set_local[rand_id[0], :, :, :] # (1,way,n_query,way)
        pred_query_set_local_2 = pred_query_set_local[rand_id[1], :, :, :] # (1,way,n_query,way)
        pred_query_set_local_1 = pred_query_set_local_1.contiguous().view(params.n_way*params.n_query, params.n_way) # (6*way,way)
        pred_query_set_local_2 = pred_query_set_local_2.contiguous().view(params.n_way*params.n_query, params.n_way) # (6*way,way)
        
        self_image_local_loss = torch.mean(torch.sum(torch.log((pred_query_set_local_1  + eps)**(-pred_query_set_local_2)), dim=1))
        #################
        ...
        rand_id_global = np.random.permutation(params.n_query) # random pick one image in the query set
        ...
        rand_id_local_sample = np.random.permutation(params.n_query) # random pick one image in the query set (issue: may be the same one of global)
        pred_query_set_local = pred_query_set_aug.view(6, params.n_way, params.n_query, params.n_way)
        ...
        loss = ce_loss + self_image_loss * params.lamba1 + cross_image_loss * params.lamba2 + self_image_local_loss * params.lamba3
        _, predicted = torch.max(pred_query_set[0].data, 1) # compute metrics with global query set prediction
        ...
        #     print(name, torch.isnan(parameter.detach().cpu() if parameter is not None else torch.Tensor([0])).any(),
        #           torch.isnan(parameter.grad.detach().cpu() if parameter.grad is not None else torch.Tensor([0])))
        ############################
        optimizer.step()
        ...
        local_loss = local_loss + self_image_local_loss.item()

    avg_loss = total_loss/float(i+1)
    avg_cls_loss = cls_loss/float(i+1)
    avg_self_loss = self_loss/float(i+1)
    avg_cross_loss = cross_loss/float(i+1)
    avg_local_loss = local_loss/float(i+1)
    return avg_loss, avg_cls_loss, avg_self_loss, avg_cross_loss, avg_local_loss, top1.avg
...
if __name__=='__main__':
    ...
    for epoch in range(params.epoch):
        print(f"Epoch: {epoch},", int(torch.cuda.memory_allocated()/1024/1024))
        train_loss, cls_loss, self_loss, cross_loss, local_loss, train_acc = train(train_loader, model, Siamese_model, head, loss_fn, optimizer, params)
        print('train epoch:', epoch+1, 
              'train loss | cls loss | self loss | cross loss | local_loss | train acc\n', 
              f'{train_loss}, {cls_loss}, {self_loss}, {cross_loss}, {local_loss}, {train_acc}')
        train_loss_all.append(train_loss)
        cls_loss_all.append(cls_loss)
        self_loss_all.append(self_loss)
        cross_loss_all.append(cross_loss)
        local_loss_all.append(local_loss)

        ## TODO: save model with best train_acc
    ...
    plt.plot(cls_loss_all, label='cls_loss')
    plt.plot(self_loss_all,label='self_loss')
    plt.plot(cross_loss_all,label='cross_loss')
    plt.plot(local_loss_all,label='local_losss')
