...
class SetDataset:
    def __init__(self, data_path, num_class, batch_size):
        ...
        #     print (len(self.sub_meta[key])) # 600
...
class SubDataset:
    ...
    def __getitem__(self,i):
        ...
        img = self.sub_meta[i] 
        # 8(2 張 224 * 224 + 6 張 96 * 96) * 3 * 224 * 224 
        multi_crops = list(map(lambda trans: trans(img), self.trans))
        # raw_image dim: 3*224*224
        raw_image = self.global_transforms(img)
        # multi_crops: including 8 patches and one raw input image
...
if __name__ == '__main__':
    ...
    datamgr = Eposide_DataManager(data_path=data_path, num_class=64, n_way=8, n_support=5, n_query=15, n_eposide=2)
    base_loader = datamgr.get_data_loader()
    data = []
    for i, x in enumerate(base_loader):
        print(i, len(x), x[0].shape) # 9 torch.Size([5, 20, 3, 224, 224])
