...
def train(train_loader, model, Siamese_model, head, discriminator, loss_fn, optimizer, params):
    model.train()
    discriminator.train()
    ...
    for i, x in enumerate(tqdm(train_loader)):
        ...
        ################# add global domain discriminator loss ###############
        global_adversarial_loss = 0.0
        shape_list = [1 for i in range(params.n_support)]
        for i in range(params.n_way):
            print("support_set_anchor[i]", support_set_anchor[i].shape)
            discriminator_out = discriminator(support_set_anchor[i]) # (way, shot, "num_classes")
            print("discriminator_out", discriminator_out.shape, discriminator_out[0])
            global_adversarial_loss += Real_AdLoss(discriminator_out, loss_fn, shape_list)
        ###################
        ...
        loss = ce_loss + self_image_loss * params.lamba1 + cross_image_loss * params.lamba2 + global_adversarial_loss * params.lamba3
        ...
        adversarial_loss = local_loss + global_adversarial_loss.item()
    ...
    avg_adversarial_loss = adversarial_loss/float(i+1)
    return avg_loss, avg_cls_loss, avg_self_loss, avg_cross_loss, avg_adversarial_loss, top1.avg
...
if __name__=='__main__':
    ...
    head = head.cuda()
    discriminator = Discriminator(n_classes=params.n_support).cuda()
    ...
    self_loss_all = []
    cross_loss_all = []
    adversarial_loss_all = []
    best_acc = 0.0
    for epoch in range(params.epoch):
        print(f"Epoch: {epoch}")
        train_loss, cls_loss, self_loss, cross_loss, adversarial_loss, train_acc = train(train_loader, model, Siamese_model, head, discriminator, loss_fn, optimizer, params)
        print('train epoch:', epoch+1, 
              'train loss | cls loss | self loss | cross loss | adversarial_loss | train acc\n', 
              f'{train_loss}, {cls_loss}, {self_loss}, {cross_loss}, {adversarial_loss}, {train_acc}')
        train_loss_all.append(train_loss)
        cls_loss_all.append(cls_loss)
        self_loss_all.append(self_loss)
        cross_loss_all.append(cross_loss)
        adversarial_loss_all.append(adversarial_loss)
    ...
    plt.plot(adversarial_loss_all,label='adversarial_loss')
